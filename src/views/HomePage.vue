...
onMounted(async () => {
  // is valid url?
  const urlParam: string | URL = route.params.url as string | URL;
  const clipboardText = await navigator.clipboard.readText()

  const validPasteUrl = validateUrl(clipboardText)
  const validPageUrl = validateUrl(urlParam)

  if (validPasteUrl) url.value = clipboardText
  else if (validPageUrl) url.value = urlParam
})
...
<template>
  <div class="animated-bg min-h-screen min-w-screen flex items-center justify-center p-4">
